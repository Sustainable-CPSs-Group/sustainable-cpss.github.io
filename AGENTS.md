...
> **This repo is a deployed site, not the upstream al-folio starter.** It is the website of the
> TUM-IAS Sustainable Cyber-Physical Systems Focus Group. Where this document describes upstream
> starter policy, the deviations below apply and take precedence.
...
`npm run lint:style-contract` checks for any of them here, and also rejects `build:css` / `build:tailwind` npm scripts — but nothing in CI invokes it any more (see below). Do not add a starter-local Tailwind or CSS build pipeline.
...
### Why the stop sign still applies

`unit-tests.yml` — which ran `npm run lint:style-contract` — was removed with the rest of the
upstream-maintenance CI, so nothing mechanically enforces the forbidden paths above any more. Keep
respecting them anyway: shadowing a gem-owned layout, include or Sass partial is what makes
`bundle exec al-folio upgrade apply` painful later. `test/style_contract.js` is still on disk and
can be run by hand with `npm run lint:style-contract`.
...
3. **This site's baseurl is empty.** It is an organization GitHub Pages site served from
   `https://sustainable-cpss-group.github.io/`, so `_config.yml` sets `baseurl: ""`. A plain
   `bundle exec jekyll build` is correct; that is what `deploy.yml` runs. Do **not** pass
   `--baseurl /al-folio` and do not restore that value — doing so prefixes every link and asset
   with a path that does not exist. The dev server is at `http://localhost:8080/` via
   `docker compose up`.

## Validated local command set

Ruby and Bundler are deliberately not installed on the maintainer's host, so `bundle exec` commands
run inside the container: `docker compose exec jekyll bundle exec …`. Run from the repo root, in
this order:
...
docker compose exec jekyll bundle exec al-folio upgrade report
docker compose down
```

The seven `test/integration_*.sh` scripts and `npm run test:visual` still exist on disk but are no
longer run by any workflow (`unit-tests.yml` and `visual-regression.yml` were both removed); run
them by hand only if your change touches that area. Docker note: v1 uses
`/srv/jekyll/bin/entry_point.sh` and serves from container-local `/tmp/_site` to avoid host
bind-mount write deadlocks.
...
- If you create or keep local overrides of plugin-owned files, run `docker compose exec jekyll bundle exec al-folio upgrade overrides audit` and commit `.al-folio-overrides.yml` after review.
